...
public class RegistryHelperImpl {
    ...
    public static  <T extends EntityType<?>> Supplier<T> registerEntity(String name, Supplier<T> entityType) {
        if (name.equals("duck")) {
            entityType = () -> (T) EntityType.Builder.create(DuckEntityForge::new, CustomSpawnGroup.WATERFOWL.spawnGroup).dimensions(0.6f, 0.6f).maxTrackingRange(10).build(DuckMod.stringID("duck"));
        } else if (name.equals("goose")) {
            entityType = () -> (T) EntityType.Builder.create(GooseEntityForge::new, CustomSpawnGroup.WATERFOWL.spawnGroup).dimensions(0.7f, 1.2f).maxTrackingRange(10).build(DuckMod.stringID("goose"));
        }
